...
export type { PriceTier, Feature, BillingCycle };
...
const PricingComponent: React.FC<PricingComponentProps> = ({
    ...
    <div className="flex justify-center mb-10 mt-2">
      ...
      >
        ...
        <ToggleGroupItem
          ...
          className="px-6 py-1.5 text-sm font-medium data-[state=on]:bg-background data-[state=on]:shadow-sm data-[state=on]:border data-[state=on]:ring-1 data-[state=on]:ring-ring/20 rounded-md transition-colors relative group"
        >
          Annually
          <motion.span 
            initial={{ scale: 0.8, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="absolute -top-4 -right-12 text-[10px] font-black text-white bg-indigo-600 px-2 py-0.5 rounded-full shadow-lg border border-white/20 whitespace-nowrap z-10"
          >
      ...
      {plans.map((plan) => {
        ...
        return (
          <Card
            key={plan.id}
            className={cn(
              "flex flex-col transition-all duration-500 shadow-md hover:shadow-2xl dark:hover:shadow-white/10",
            ...
            <CardHeader className="p-6 pb-4">
              ...
              <div className="mt-4 h-20">
                <AnimatePresence mode="wait">
                  ...
                  >
                    <p className="text-4xl font-extrabold text-foreground flex items-baseline gap-1">
                      ₹{currentPrice}
                    ...
                    {billingCycle === 'annually' && plan.priceAnnually > 0 && (
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <p className="text-[10px] text-indigo-600 font-bold bg-indigo-50 px-1.5 rounded">
                            Billed ₹{plan.priceAnnually}/yr
                        </p>
                      </div>
                    )}
                  </motion.div>
  ...
  // --- 3.4. Final Render ---
  return (
    <div className={cn("w-full py-0 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8", className)} {...props}>
      {CycleToggle}
