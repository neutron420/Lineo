  ...
  const handleOrgSearch = async () => {
    ...
    try {
      ...
      if (data.status === "OK" && data.results.length > 0) {
        ...
        let detected = nearbyResp.data.data || [];
        
        // Mock data if environment is empty (for demo)
        if (detected.length === 0) {
          detected = [
            { id: 1, name: "City General Hospital", type: "hospital" },
            { id: 2, name: "Prudent Reserve Bank", type: "bank" },
            { id: 3, name: "District Civic Center", type: "government" }
          ];
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      await api.post("/auth/register-org", {
        ...
        cert_pdf_url: formData.certPdfURL || formData.certPdfURL,
        ptax_paper_url: formData.ptaxPaperURL || formData.ptaxPaperURL,
        turnstile_token: captchaToken
      });

      toast.success("Registration Successful!", {
        description: "Your organization is now pending verification. We will review your documents shortly."
      });
  ...
  return (
    <div className="min-h-screen bg-[#f7fafd] flex flex-col items-center justify-center p-8 selection:bg-[#493ee5]/10 selection:text-[#493ee5]">
      ...
      <div className="w-full max-w-[850px]">
        <Link href="/register" className="mb-8 text-[#49607e] hover:text-[#181c1e] transition-colors flex items-center gap-2 group font-bold text-sm">
          <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" /> Back to choices
        </Link>
            ...
            {currentStep === 1 && (
              ...
              >
                ...
                <div className="space-y-8">
                         ...
                         <div className="bg-white rounded-[32px] p-2 border border-[#e5e8eb] shadow-2xl h-full flex flex-col relative overflow-hidden group">
                            {isLoaded ? (
                              <div className="w-full h-[300px] rounded-[28px] overflow-hidden relative">
                                ...
                                {/* Mini Lat/Long Overlay */}
                                <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-md border border-[#e5e8eb] p-3 rounded-2xl flex items-center justify-between shadow-lg">
                                   <div className="flex gap-4">
                                      <div className="flex flex-col">
                                         <span className="text-[8px] font-black uppercase text-[#49607e] tracking-widest">Lat</span>
                                         <span className="text-[10px] font-bold text-[#181c1e] font-mono">{formData.lat.toFixed(5)}</span>
                                      </div>
                                      <div className="flex flex-col">
                                         <span className="text-[8px] font-black uppercase text-[#49607e] tracking-widest">Lng</span>
                                         <span className="text-[10px] font-bold text-[#181c1e] font-mono">{formData.lng.toFixed(5)}</span>
                                      </div>
                                   </div>
                                   <div className="flex items-center gap-1.5 bg-emerald-50 px-2 py-1 rounded-lg">
                                      <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                                      <span className="text-[8px] font-black text-emerald-600 uppercase tracking-widest">Locked</span>
                                   </div>
                                </div>
                              </div>
                            ) : (
                   ...
                   {nearbyOrgs.length > 0 && (
                     <div className="p-8 bg-[#493ee5]/5 border border-[#493ee5]/10 rounded-[32px] animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="flex items-center justify-between mb-6">
                           <div className="flex items-center gap-3">
                              <Users className="w-5 h-5 text-[#493ee5]" />
                              <span className="text-[10px] font-black text-[#493ee5] uppercase tracking-[0.2em]">Nearby Institutional Network</span>
                           </div>
                           <span className="text-[9px] font-bold text-[#49607e] uppercase">Click to Auto-sync details</span>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                           {nearbyOrgs.map((org, i) => (
                              <button 
                                ...
                                className="px-5 py-4 bg-white border border-[#e5e8eb] rounded-2xl text-[12px] font-black text-[#181c1e] shadow-sm flex items-center justify-between hover:border-[#493ee5] hover:bg-[#493ee5]/5 hover:translate-y-[-2px] transition-all group"
                              >
                                 <div className="flex items-center gap-3">
                                    <div className="w-2 h-2 bg-emerald-500 rounded-full group-hover:scale-125 transition-transform" />
                                    {org.name}
                                 </div>
                                 <ArrowRight className="w-4 h-4 text-[#49607e] opacity-0 group-hover:opacity-100 transition-all group-hover:translate-x-1" />
                              </button>
                           ))}
                        </div>
                     </div>
       ...
       <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
             ...
             <p className="text-[10px] text-[#49607e] font-medium break-all max-w-[200px]">
               {value ? (
                 <span className="text-emerald-600 font-bold">Synchronized to Cloud Cloud</span>
               ) : (
                 "Action Required"
               )}
             </p>
