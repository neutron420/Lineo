...
export default function UserDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [activeResp, apptResp, nearbyResp] = await Promise.all([
        api.get("/queue/active").catch(err => {
          console.warn("Active ticket info unavailable:", err.message);
          return { data: { data: null } };
        }),
        api.get("/appointments").catch(err => {
          console.warn("Appointments unavailable:", err.message);
          return { data: { data: [] } };
        }),
        api.get(`/search/nearby?lat=${coords.lat}&lng=${coords.lng}${categoryParam}`).catch(err => {
          console.warn("Nearby discovery unavailable:", err.message);
          return { data: { data: [] } };
        })
      ]);

      setActiveToken(activeResp.data.data);
      setAppointments(apptResp.data.data || []);
      setNearbyOrgs(nearbyResp.data.data || []);
    } catch (err) {
    ...
    <div className="space-y-12 text-left pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-stripe-slate text-xl font-light mt-1">Real-time status of your queues and appointments.</p>
        </div>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-6">
            ...
            >
              ...
              <div className="mb-10 text-left">
                <h3 className="text-3xl font-bold text-stripe-navy tracking-tight">Access Institutional Queue</h3>
                <p className="text-stripe-slate mt-2 text-lg font-light">Secure your remote spot using the institutional identifier.</p>
              </div>

              <form onSubmit={handleJoinQueue} className="space-y-8">
                <div className="space-y-4 text-left">
                   <label className="text-[12px] font-extrabold text-stripe-slate uppercase tracking-[0.15em] pl-2">Institution Code</label>
                   <div className="relative group">
                      <Search className="absolute left-6 top-1/2 -translate-y-1/2 w-6 h-6 text-stripe-slate group-focus-within:text-stripe-purple transition-colors" />
                      <input 
                        type="text" 
                        required
                        autoFocus
                        value={joinQueueKey}
                        onChange={(e) => setJoinQueueKey(e.target.value.toUpperCase())}
                        placeholder="e.g. SBI-MAIN-01" 
                        className="w-full pl-16 pr-6 py-6 bg-[#f6f9fc] border-2 border-transparent rounded-[24px] text-[20px] font-extrabold tracking-wider focus:bg-white focus:border-stripe-purple/30 focus:ring-8 focus:ring-stripe-purple/5 transition-all outline-none"
                      />
                   </div>
                </div>
                ...
                <div className="pt-6 flex flex-col gap-5">
                   <button 
                     ...
                     className="stripe-btn-primary py-6 w-full text-lg font-bold shadow-2xl shadow-stripe-purple/30 flex items-center justify-center gap-4 hover:scale-[1.02] active:scale-[0.98] transition-all"
                   >
                     {isSubmitting ? <Loader2 className="w-6 h-6 animate-spin" /> : <><Zap className="w-6 h-6" /> Confirm Remote Access</>}
                   </button>
                   <button 
                     type="button"
                     onClick={() => setIsJoinModalOpen(false)}
